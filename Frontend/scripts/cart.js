var cartItemsArr = JSON.parse(localStorage.getItem("cartArrayS1")) || [];
  ...
  function displayArr(cartItemsArr) {
    cartItemsArr.map((element) => {
      ...
      plus.addEventListener("click", function () {
        let arr = JSON.parse(localStorage.getItem("cartArrayS1"));
        count++;
        price_h2.textContent = `Rs. ${+mrp * count}`;
        cnt.textContent = count;
        total_price = +mrp * count;
